...
def process_sms_client_response(self, status, provider_reference, client_name, detailed_status_code=None):
    # Twilio generate their own reference (SID), rather than being generated by us via UUID, hence we must skip this check
    if not client_name == 'Twilio':
        try: # Validate SMS reference received in the callback; it should be a valid UUID
...
def _process_for_status(notification_status, client_name, provider_reference, detailed_status_code=None):
    ...
    if client_name == 'Twilio':
        notification = notifications_dao.update_notification_status_by_reference(reference=provider_reference,status=notification_status)
    else:
